 ...
 */
package org.breedinginsight.brapps.importer.imports.germplasm;
...
import org.breedinginsight.brapps.importer.imports.germplasm.model.Germplasm;
import org.breedinginsight.brapps.importer.base.model.BrAPIImport;
import org.breedinginsight.brapps.importer.base.model.response.PendingImport;
import org.breedinginsight.brapps.importer.base.model.response.ImportObjectState;
...
public class GermplasmProcessor implements Processor {
    ...
    public void getExistingBrapiData(List<BrAPIImport> importRows, Program program) throws ApiException {
        ...
        List<GermplasmImport> germplasmImportRows = importRows.stream()
                .map(row -> (GermplasmImport) row)
                .collect(Collectors.toList());

        // Get all of our objects specified in the data file by their unique attributes
        Set<String> germplasmDBIDs = new HashSet<>();
        for (int i = 0; i < germplasmImportRows.size(); i++) {
            GermplasmImport germplasmImport = germplasmImportRows.get(i);
        ...
        if (importRows.size() > 0 && germplasmImportRows.get(0).getGermplasm().getListName() != null) {
            try {
                Germplasm row = germplasmImportRows.get(0).getGermplasm();
        ...
        for (BrAPIImport brAPIImportRow: importRows) {
            GermplasmImport importRow = (GermplasmImport) brAPIImportRow;
    ...
    public void process(List<BrAPIImport> importRows, Program program, User user, boolean commit,
        ...
        if (commit) {
            Germplasm germplasm = germplasmImportRows.get(0).getGermplasm();
        ...
        for (int i = 0; i < germplasmImportRows.size(); i++) {
            GermplasmImport brapiImport = germplasmImportRows.get(i);
        ...
        ImportPreviewStatistics pedigreeConnectStats = ImportPreviewStatistics.builder()
                .newObjectCount(germplasmImportRows.stream().filter(germplasmImport ->
    ...
    public void constructPedigreeString(List<BrAPIImport> importRows, Map<Integer, PendingImport> mappedBrAPIImport, Boolean commit) {
        ...
        for (int i = 0; i < importRows.size(); i++) {
            GermplasmImport brapiImport = (GermplasmImport) importRows.get(i);
            Germplasm germplasm = brapiImport.getGermplasm();
