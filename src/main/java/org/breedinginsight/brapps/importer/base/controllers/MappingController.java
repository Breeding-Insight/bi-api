...
package org.breedinginsight.brapps.importer.base.controllers;
...
import org.breedinginsight.brapps.importer.base.services.MappingService;
...
public class MappingController {
    ...
    @Inject
    MappingController(SecurityService securityService, MappingService mappingService) {
    ...
    public HttpResponse<Response<DataResponse<ImportConfigResponse>>> getImportTemplates() {
        List<ImportConfigResponse> configs = mappingService.getAllImportTypeConfigs();
    ...
    public HttpResponse<Response<DataResponse<ImportMapping>>> getMappings(@PathVariable UUID programId) {
        ...
        try {
            ...
            List<ImportMapping> result = mappingService.getAllMappings(programId, actingUser);
            List<Status> metadataStatus = new ArrayList<>();
            metadataStatus.add(new Status(StatusCode.INFO, "Successful Query"));
    ...
    public HttpResponse<Response<ImportMapping>> getMappingDetails(@PathVariable UUID programId, @PathVariable UUID mappingId) {
        ...
        try {
            ImportMapping result = mappingService.getMappingDetails(mappingId);
    ...
    public HttpResponse<Response<ImportMapping>> createMapping(@PathVariable UUID programId, @Body ImportMapping mapping) {
        try {
            AuthenticatedUser actingUser = securityService.getUser();
            ImportMapping result = mappingService.createMapping(programId, actingUser, mapping);
    ...
    public HttpResponse<Response<ImportMapping>> updateMapping(@PathVariable UUID programId, @PathVariable UUID mappingId,
        ...
        try {
            AuthenticatedUser actingUser = securityService.getUser();
            ImportMapping result = mappingService.updateMapping(programId, actingUser, mappingId, mapping);
            Response<ImportMapping> response = new Response(result);
            return HttpResponse.ok(response);
        } catch (DoesNotExistException e) {
            log.error(e.getMessage(), e);
